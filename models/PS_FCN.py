...
class PS_FCN_Model_Param(nn.Module):
    '''仅用来测试模型的相关参数：如FPS、Params、内存占用、FLOPs'''

    # 因为仅测试模型参数，故输入大小为一个torch格式张量，大小为[Batchsize, 2*N*3, H, W]

    def __init__(self, fuse_type='max', batchNorm=False, c_in=3, other={}):
        super(PS_FCN_Model_Param, self).__init__()
        self.extractor = FeatExtractor(batchNorm, c_in, other)
        self.regressor = Regressor(batchNorm, other)
        self.c_in = c_in
        self.fuse_type = fuse_type
    ...
    def forward(self, x):
        img_split = torch.split(x, 6, 1)
        feats = []
        for i in range(len(img_split)):
            feat, shape = self.extractor(img_split[i])
            feats.append(feat)
        if self.fuse_type == 'mean':
            feat_fused = torch.stack(feats, 1).mean(1)
        elif self.fuse_type == 'max':
