...
# 面对Blobby、Sculpture数据集时采用，此时不需要记录每一个类别的名字
...
# 面对DiliGent数据集（不包含DiLiGent_102、DiLiGent_Pi）时采用，此时需要记录每一个类别的名字
def parseData_DiliGent(args, sample, timer=None, split='train'):
    input, target, mask, obj_name = sample['img'], sample['N'], sample['mask'], sample['obj']
    if timer: timer.updateTime('ToCPU')
    if args.cuda:
        device = torch.device(args.device)
        input = input.to(device)
    ...
    mask_var = torch.autograd.Variable(mask, requires_grad=False)
    if timer: timer.updateTime('ToGPU')
    data = {'input': input_var, 'tar': target_var, 'm': mask_var, 'obj': obj_name}
...
def saveCheckpoint(save_path, epoch=-1, model=None, optimizer=None, records=None, args=None):
    ...
    torch.save(state, os.path.join(save_path, 'checkp_best.pth.tar'))
    torch.save(records, os.path.join(save_path, 'checkp_best_rec.pth.tar'))
...
def DilatedConvolution(batchNorm, cin, cout, k=3, dilation=1, pad=-1):
    pad = (k - 1) // 2 if pad < 0 else pad
    # print('Conv pad = %d' % (pad))
    if batchNorm:
        print('=> convolutional layer with bachnorm')
        return nn.Sequential(
            nn.Conv2d(cin, cout, kernel_size=k, dilation=dilation, padding=pad, bias=False),
            nn.BatchNorm2d(cout),
            nn.LeakyReLU(0.1, inplace=True)
        )
    else:
        return nn.Sequential(
            nn.Conv2d(cin, cout, kernel_size=k, dilation=dilation, padding=pad, bias=True),
            nn.LeakyReLU(0.1, inplace=True)
        )
